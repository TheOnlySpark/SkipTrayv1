-- 0013_lunch_slot_timings_and_sunday_restriction.sql
-- Enforces:
-- 1. Lunch pickup slots only from 12:30 PM to 1:40 PM in 10-minute intervals
-- 2. Orders must be placed at least 30 minutes in advance of the pickup slot on the same date
-- 3. No orders on Sunday (canteen closed)
...
BEGIN
  ...
  IF EXTRACT(DOW FROM NOW() AT TIME ZONE 'Asia/Kolkata') = 0 THEN
  ...
  IF ((NOW() AT TIME ZONE 'Asia/Kolkata')::date + p_pickup_time::time) < ((NOW() AT TIME ZONE 'Asia/Kolkata') + INTERVAL '29 minutes') THEN
